...
To see this, consider our EEO example data. We first standardize the variables (so we can omit the ones column in the design matrix) using the `scale()` function. Then, we will compute the condition number for the $\mathbf{X}^{\intercal}\mathbf{X}$ matrix.
...
We can also use the `lm.ridge()` function from the `{MASS}` package to fit a ridge regression. This function uses a formula based on variables from a data frame in the same fashion as the `lm()` function. It also takes the argument `lambda=` which specifies the values of $\lambda$ to use in the penalty term.

To avoid using `scale()` on every variable, we standardize all the variables in th data simultaneously by piping the dat into the `scale()` function. Note that the output of the `scale()` function is a matrix and because the `data=` argument has to be a data frame (or tibble), we convert the standardized data (which is a matrix) to a data frame using the `data.frame()` function.
...
How do these biased coefficients from the ridge regression compare to the unbiased coefficients from the OLS estimation? Below, we grab the coefficients from the original standardized OLS model to the data, and compare the coefficients to those from the ridge regression with $\lambda=0.1$.
...
ridge_1 = lm.ridge(achievement ~ 1 + faculty + peer + school, data = z_eeo, lambda =  0.1)
ridge_2 = lm.ridge(achievement ~ 1 + faculty + peer + school, data = z_eeo, lambda =  1  )
ridge_3 = lm.ridge(achievement ~ 1 + faculty + peer + school, data = z_eeo, lambda = 10  )

cn_0 = abs(max(eigen(t(X) %*% X)$values)) / abs(min(eigen(t(X) %*% X)$values))
cn_1 = abs(max(eigen(t(X) %*% X + 0.1*diag(4))$values)) / abs(min(eigen(t(X) %*% X + 0.1*diag(4))$values))
cn_2 = abs(max(eigen(t(X) %*% X + 1*diag(4))$values)) / abs(min(eigen(t(X) %*% X + 1*diag(4))$values))
cn_3 = abs(max(eigen(t(X) %*% X + 10*diag(4))$values)) / abs(min(eigen(t(X) %*% X + 10*diag(4))$values))


tab_02 = data.frame(
  B =  c("Faculty", "Peer", "School", "Condition Number"),
  x1 = c(coef(lm.z), cn_0)[-1],
  x2 = c(coef(ridge_1), cn_1)[-1],
  x3 = c(coef(ridge_2), cn_2)[-1],
  x4 = c(coef(ridge_3), cn_3)[-1]
)
...
ridge_models = lm.ridge(achievement ~ 1 + faculty + peer + school, data = z_eeo, 
...
# COnvert to data frame to get all decimal places
my_models |> 
  filter(AIC == min(AIC)) |> 
  data.frame()
```
...
A $\lambda$ value of 43.97 produces the smallest AIC value, so this is the $\lambda$ value we will adopt.


```{r}
# Re-fit ridge regression using lambda = 43.97
...
Based on using $\lambda=43.97$, the fitted ridge regression model is:

$$
\hat{\mathrm{Achievement}}^{\star}_i = 0.110(\mathrm{Faculty}^{\star}_i) + 0.141(\mathrm{Peer}^{\star}_i) + 0.103(\mathrm{School}^{\star}_i)
$$
...
#| fig-cap: "Ridge plot showing the size of the standardized regression coefficients for $\\lambda$ values between 0 (OLS) and 100. The vertical dotted line is shown at $\\lambda=43.97$, the value of $\\lambda$ that has the minimum GCV value."
#| fig-alt: "Ridge plot showing the size of the standardized regression coefficients for $\\lambda$ values between 0 (OLS) and 100. The vertical dotted line is shown at $\\lambda=43.97$, the value of $\\lambda$ that has the minimum GCV value."
#| fig-width: 8
#| fig-height: 6
#| out-width: "70%"
...
```{r}
#| label: tbl-comp3
#| tbl-cap: "Comparison of the standard errors from the OLS (&lambda;=0) and ridge regression (&lambda;;=43.97) based on the standardized data."
#| echo: false

tab_01 = data.frame(
  Predictor = c("Faculty", "Peer", "School"),
  OLS = tidy(lm(achievement ~ 1 + faculty + peer + school, data = z_eeo))$std.error[-1],
  Ridge = sqrt(diag(var_b))[-1]
)
...
This suggests that after controlling for peer influence and faculty credentiala, there is evidence of an effect of school facilities on student achievement ($p=.003$). The uncertainty in the 95% CI suggests that the true partial effect of school facilities is between 0.037 and 0.169. The empirical evidence is pointing toward a slight positive effect of school facilities.
